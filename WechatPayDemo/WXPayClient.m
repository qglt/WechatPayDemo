...
- (NSString *)genPackage
{
    ...
    [params setObject:@"青格勒图" forKey:@"body"];
    ...
    [params setObject:@"200" forKey:@"total_fee"];    // 1 =＝ ¥0.01
...
- (NSMutableData *)getProductArgs
{
    ...
    self.nonceStr = [self genNonceStr]; // traceId ke自定义，可用于订单的查询与跟踪，建议根据支付用户信息生成此id
    ...
    [self.request setCompletionBlock:^{
        ...
        NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:[weakRequest responseData] 
                                                             options:kNilOptions 
                                                               error:&error];
        if (error) {
            [weakSelf showAlertWithTitle:@"错误" msg:@"支付请求失败！"];
            return;
        }
